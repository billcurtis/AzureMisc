...
Write-Output "Running Windows Image Health Check..."

if ($UploadToStorageAccount) {
    Write-Output "Upload Policy: Upload results to Azure Storage Account"
}
else {
    Write-Output "Upload Policy: Local processing only - no upload"
}

# Run health check
Write-Output "Running Windows Image Health Scan..."
try {
    $imageHealthResult = Repair-WindowsImage -Online -ScanHealth -ErrorAction Stop
    
    if (!$imageHealthResult) {
        Write-Output "Warning: Image health scan returned null result"
        $imageHealthResult = @{
    ...
    elseif ($null -eq $imageHealthResult.ImageHealthState.ToString() -or $imageHealthResult.ImageHealthState.ToString() -eq "") {
        Write-Output "Warning: Image health state is null or empty"
        $imageHealthResult.ImageHealthState = "Unknown"
    }
    
    Write-Output "Health check completed: $($imageHealthResult.ImageHealthState.ToString())"
}
catch {
    Write-Output "Error: Image health scan failed: $($_.Exception.Message)"
    $imageHealthResult = @{
        ImageHealthState = "ScanFailed"
        RestartNeeded = $false
    }
    Write-Output "Using fallback health state: ScanFailed"
}
...
if ($imageHealthResult.ImageHealthState -eq "Repairable") {
    Write-Output "Image is repairable. Running Repair-WindowsImage -Online -RestoreHealth..."
    try {
        Repair-WindowsImage -Online -RestoreHealth | Out-Null
        Write-Output "Image repair completed successfully"
        $imageRepaired = $true
    }
    catch {
        Write-Output "Image repair failed: $($_.Exception.Message)"
        $imageRepaired = $false
    }
}
...
try {
        ...
        if ($UpdatesToInstall.Count -gt 0) {
            ...
            foreach ($Update in $SearchResult.Updates) {
                try {
                    ...
                    Write-Output "  Error processing update $($Update.Title): $($_.Exception.Message)"
    ...
    Write-Output "Windows Update check/install failed: $($_.Exception.Message)"
    ...
    try {
        ...
        if ($updatesInstalled.Count -gt 0) {
            try {
                ...
                Write-Output "Warning: Failed to convert updates to JSON, using empty array"
                $updatesJson = "[]"
            }
        }
        ...
        if ($updatesFound.Count -gt 0) {
            try {
                ...
                Write-Output "Warning: Failed to convert updates found to JSON, using empty array"
                $updatesFoundJson = "[]"
            }
        }
        ...
        if ($updatesFailed.Count -gt 0) {
            try {
                ...
                Write-Output "Warning: Failed to convert update failures to JSON, using empty array"
                $updatesFailedJson = "[]"
            }
        }
        ...
        try {
            ...
            Write-Output "Error: Failed to convert entity to JSON: $($_.Exception.Message)"
            Write-Output "Attempting manual JSON creation..."
...
"@
        ...
        try {
            ...
            Write-Output "Warning: Failed to calculate content length, using string length"
        ...
        try {
            ...
            Write-Output "UPLOAD SUCCESS: Health: $($entityData.ImageHealthState), Restart: $($entityData.RestartNeeded), Updates Found: $($updatesFound.Count), Updates Installed: $($updatesInstalled.Count), Updates Failed: $($updatesFailed.Count), Image Repaired: $imageRepaired, Reboot Required: $rebootRequired"
        }
        catch {
        ...
        Write-Output "UPLOAD PREPARATION FAILED: $($_.Exception.Message)"
