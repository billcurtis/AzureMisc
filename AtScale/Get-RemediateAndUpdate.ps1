...
.DESCRIPTION
    ...
    Key Features:
    - Intelligent user session detection with configurable bypass option to prevent execution during active user sessions
...
if ($SkipUserSessionCheck) {
    ...
    if ($loggedOnUsers.Count -gt 0) {
        # Users are logged on - skip all operations except logging and upload
...
"@
        ...
        try {
            ...
            Write-Log "UPLOAD SUCCESS: Health: $($entityData.ImageHealthState), Restart: $($entityData.RestartNeeded), Updates Found: $($updatesFound.Count), Updates Installed: $($updatesInstalled.Count), Updates Failed: $($updatesFailed.Count), Image Repaired: $imageRepaired, After Repair Status: $(if ($afterRepairStatus) { $afterRepairStatus } else { 'N/A' }), Reboot Required: $rebootRequired" "INFO"
        }
        catch {
    ...
    Write-Output "SKIPPED UPLOAD: Health: $($imageHealthResult.ImageHealthState.ToString()), Restart: $($imageHealthResult.RestartNeeded.ToString()), Updates Found: $($updatesFound.Count), Updates Installed: $($updatesInstalled.Count), Updates Failed: $($updatesFailed.Count), Image Repaired: $imageRepaired, After Repair Status: $(if ($afterRepairStatus) { $afterRepairStatus } else { 'N/A' }), Reboot Required: $rebootRequired"
...
Write-Output "  Health State: $($imageHealthResult.ImageHealthState.ToString())"
Write-Output "  Restart Needed: $($imageHealthResult.RestartNeeded.ToString())"
